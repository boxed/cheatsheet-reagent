...
(defn parse-link [t]
  (if (nil? t)
    ""
    (let [[_ text1 url text2 colon] (re-matches #"(.* )?\(?<a href=\"(.*)\">(.*)</a>\)?(:?)" t)]
      (if (nil? url)
        {:text (-> t
                   (clojure.string/replace #"</?var>" "")
                   (clojure.string/replace #"</?code>" "")
                   (clojure.string/replace "&rarr;" "→")
                   (clojure.string/replace "<br>" "\n"))}
        (if (nil? text1)
          {:url url, :text (apply str [text2 colon])}
          {:url url, :text (apply str [text1 "(" text2 ")"])})))))
...
(defn parse-cmd [cmd]
  (match cmd
         ...
         [:common-prefix prefix & cmds] (for [c cmds] (parse-cmd (apply str [prefix c])))
         [:common-suffix suffix & cmds] (for [c cmds] (parse-cmd (apply str [c suffix])))
         [:common-prefix-suffix prefix suffix & cmds] (for [c cmds] (parse-cmd (apply str [prefix c suffix])))
         :else {:text (str cmd)}
       ))
...
(defn parse-cmds [cmds]
  (if (string? cmds)
    [(parse-cmd cmds)]
    (flatten (map parse-cmd cmds))))

(defn parse-table-row [r]
  (let [[title cmds] (match r
                            [{:html title} :str {:html cmds}]      [title (:text (parse-link cmds))]
                            [{:html title} :cmds cmds]             [title cmds]
                            [{:html title} :str cmds]              [title cmds]
                            [title :cmds cmds]                     [title cmds]
                            [title :cmds-with-frenchspacing cmds]  [title cmds]
                            )]
    {:title (parse-link title)
     :cmds (parse-cmds cmds)}))

(defn parse-table [t]
  (into [] (map parse-table-row t)))
...
(defn parse-box-inner [b]
  (match b
         ...
         [:box color
          ...
          :table table] [[[section nil (parse-cmds cmds-one-line)] table]]
